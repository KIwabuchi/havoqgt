...
using graph_type = jgraph::jgraph_primitive_id<Vertex, metall::manager::allocator_type<std::byte>>;
  ...
  do {
    ...
    for (auto vitr = graph->vertices_begin(), vend = graph->vertices_end(); vitr != vend; ++vitr) {
      ...
      if (bfs_level[v_id].first == current_level) {
        ...
        for (auto eitr = graph->edges_begin(v_id),
                 eend = graph->edges_end(v_id); eitr != eend; ++eitr) {
...
int main() {
  ...
  Vertex target_vertex = 22683780; //"2283541";
